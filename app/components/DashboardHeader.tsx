  ...
  return (
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-8 relative z-10 transition-all duration-300">
      ...
      <div className="flex gap-3 w-full lg:w-auto">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-5 py-3 rounded-2xl flex-1 lg:flex-none flex flex-col justify-center items-end shadow-sm relative overflow-hidden group transition-colors duration-300">
           ...
           <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5">Equidad Total</span>
           <span className="text-xl font-black text-slate-800 dark:text-slate-100">${(equidadTotal || 0).toFixed(2)}</span>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-3 rounded-2xl flex-1 lg:flex-none flex flex-col justify-center items-end shadow-sm transition-colors duration-300">
           <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5">Capital Base</span>
           <span className="text-sm font-bold text-slate-700 dark:text-slate-300">${(saldoReal || 0).toFixed(2)}</span>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-3 rounded-2xl flex-1 lg:flex-none flex flex-col justify-center items-end shadow-sm transition-colors duration-300">
           <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5">Crédito</span>
           <span className="text-sm font-bold text-indigo-500 dark:text-indigo-400">${(bonus || 0).toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
