...
/* INICIO COMPONENTE PRINCIPAL: Calendar */
export default function Calendar({ filteredTrades, selectedMonth, onTradeUpdated }: CalendarProps) {
  ...
  /* INICIO FUNCIONES DE NAVEGACIÓN */
  const handlePrevMonth = () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1));
  const handleNextMonth = () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1));
  /* FIN FUNCIONES DE NAVEGACIÓN */

  /* INICIO CÁLCULOS DEL MOTOR DE CALENDARIO (CON DATOS PARA TOOLTIP) */
  const { daysArray, monthLabel } = useMemo(() => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      // Variables extras para el Tooltip
  ...
  /* FIN CÁLCULOS DEL MOTOR DE CALENDARIO */

  /* INICIO LÓGICA DE DÍA ACTIVO Y ELIMINACIÓN */
  ...
  /* FIN LÓGICA DE DÍA ACTIVO Y ELIMINACIÓN */

  /* INICIO RENDERIZADO VISUAL */
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 lg:p-8 shadow-sm border border-slate-100 dark:border-slate-800 w-full flex flex-col font-sans relative transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-4 border border-slate-200 dark:border-slate-700 rounded-2xl px-2 py-1 shadow-sm bg-white dark:bg-slate-800">
      ...
      <div className="grid grid-cols-7 gap-3 flex-grow">
        {daysArray.map((data, idx) => (
          <div 
            ...
            className={`
              relative flex flex-col justify-between min-h-[110px] p-3 rounded-2xl border transition-all group
          ...
          >
            ...
            <div className="flex justify-between items-start">
              <span className={`text-sm font-bold ${!data.isCurrentMonth ? 'text-slate-300 dark:text-slate-600' : data.status === 'win' ? 'text-emerald-700 dark:text-emerald-400' : data.status === 'loss' ? 'text-rose-700 dark:text-rose-400' : 'text-slate-500 dark:text-slate-400'}`}>
                {data.displayDay}
              </span>
              {data.trades.length > 0 && (
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${data.status === 'win' ? 'bg-indigo-100 dark:bg-indigo-500/20 text-indigo-500 dark:text-indigo-400' : 'bg-indigo-100 dark:bg-indigo-500/20 text-indigo-500 dark:text-indigo-400'}`}>
                  {data.trades.length}
                </span>
              )}
            </div>
            ...
            {data.trades.length > 0 && (
              <div className="text-center mt-auto mb-1">
                <div className={`text-lg font-black tracking-tight ${data.status === 'win' ? 'text-emerald-600 dark:text-emerald-500' : 'text-rose-600 dark:text-rose-500'}`}>
                  {data.net > 0 ? '+' : ''}${Math.abs(data.net).toFixed(2)}
                </div>
                <div className={`text-[9px] font-semibold mt-0.5 ${data.status === 'win' ? 'text-emerald-400 dark:text-emerald-500/70' : 'text-rose-400 dark:text-rose-500/70'}`}>
                  Neto diario
                </div>
              </div>
            )}

            {/* INICIO TOOLTIP DETALLADO (OCULTO POR DEFECTO, VISIBLE EN HOVER) */}
            {data.isCurrentMonth && data.trades.length > 0 && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block z-50 w-56 animate-in fade-in slide-in-from-bottom-2 pointer-events-none">
      ...
      <div className="mt-6 pt-4 flex justify-between items-center border-t border-slate-100 dark:border-slate-800">
        <div className="flex gap-5">
          <div className="flex items-center gap-2"><div className="w-3.5 h-3.5 rounded bg-[#ecfdf5] dark:bg-emerald-500/10 border border-[#d1fae5] dark:border-emerald-500/20"></div><span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Día Ganador</span></div>
          <div className="flex items-center gap-2"><div className="w-3.5 h-3.5 rounded bg-[#fef2f2] dark:bg-rose-500/10 border border-[#fee2e2] dark:border-rose-500/20"></div><span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Día Perdedor</span></div>
          <div className="flex items-center gap-2"><div className="w-3.5 h-3.5 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700"></div><span className="text-xs font-semibold text-slate-400 dark:text-slate-500">Sin Operaciones</span></div>
        </div>
        <span className="text-xs font-medium text-slate-400 dark:text-slate-500">Hacer clic en un día para ver detalles</span>
      </div>

      {/* INICIO POPUP PRINCIPAL DE TRADES (CLICK EN DÍA) */}
      {activeDateStr && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-150" onClick={() => { setActiveDateStr(null); setEditingTrade(null); }}>
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg">Trades del {activeDateStr}</h3>
            ...
            <div className="p-6 max-h-[400px] overflow-y-auto relative">
                  ...
                  {activeDayTrades.map((t: any) => {
                      ...
                      return (
                        <tr key={t.id} className="bg-indigo-50/50 dark:bg-indigo-900/20">
                          <td colSpan={4} className="py-3 px-4">
                            <form onSubmit={handleEditSave} className="flex items-center justify-between gap-4">
                              ...
                              <div className="flex gap-2">
                                <button type="button" onClick={() => setEditingTrade(null)} className="text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 px-3 py-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded">Cancelar</button>
                                <button type="submit" className="text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded shadow-sm">Guardar</button>
                              </div>
                            </form>
                          </td>
                        </tr>
                    ...
                    return (
                      <tr key={t.id} className="text-sm transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td className="py-4"><span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-1 rounded-md font-extrabold text-xs">{t.asset}</span></td>
                        <td className="py-4 text-slate-500 dark:text-slate-400 text-xs font-semibold">{t.setup_type}</td>
                        <td className={`py-4 text-right font-black ${net >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                          {net > 0 ? '+' : ''}${net.toFixed(2)}
                        </td>
              ...
              {/* MODAL SECUNDARIO DE CONFIRMACIÓN DE ELIMINAR */}
              {tradeToDelete && (
                <div className="absolute inset-0 z-[250] flex items-center justify-center bg-white/90 dark:bg-slate-950/90 backdrop-blur-[2px]">
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl rounded-2xl p-6 w-[80%] max-w-sm text-center">
      ...
      {/* FIN POPUP PRINCIPAL DE TRADES */}

    </div>
  );
}
/* FIN COMPONENTE PRINCIPAL: Calendar */