...
export default function MetricsCards({ metrics, winRateChartData, pfChartData, pfColor, tradesLength }: any) {
  return (
   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 md:gap-4 w-full">
      ...
      {/* 1. Ganancia Neta */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 md:p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 cursor-default">
        ...
        <div className={`text-2xl md:text-3xl font-black tabular-nums ${metrics.netPL >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
          {metrics.netPL > 0 ? '+' : ''}${metrics.netPL.toFixed(2)}
        </div>
      </div>

      {/* 2. % de Acierto */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 md:p-5 rounded-2xl shadow-sm flex flex-col justify-between relative transition-all duration-300 hover:-translate-y-1 cursor-default">
        ...
        <div className="flex justify-between items-end h-full">
          <div className="flex flex-col justify-end">
            <div className="text-2xl md:text-3xl font-black text-slate-800 dark:text-slate-100 leading-none tabular-nums">{metrics.winRate}%</div>
          ...
          <div className="w-12 h-8 md:w-14 md:h-10 relative -mr-2">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 md:p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 cursor-default">
        ...
        <div>
          <div className="flex justify-between text-xs md:text-sm font-bold mb-1 tabular-nums">
            <span className="text-emerald-500">${metrics.avgWin.toFixed(2)}</span>
            <span className="text-rose-500">-${metrics.avgLoss.toFixed(2)}</span>
          </div>
          <div className="w-full h-1.5 md:h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden flex">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 md:p-5 rounded-2xl shadow-sm flex flex-col justify-between relative transition-all duration-300 hover:-translate-y-1 cursor-default">
        ...
        <div className="flex justify-between items-end h-full">
          <div className="text-2xl md:text-3xl font-black text-slate-800 dark:text-slate-100 leading-none tabular-nums">{metrics.profitFactor}</div>
          <div className="w-12 h-8 md:w-14 md:h-10 relative -mr-2">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 md:p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 cursor-default">
        ...
        <div className="text-2xl md:text-3xl font-black text-rose-500 leading-none tabular-nums">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 md:p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 cursor-default">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full border-2 flex items-center justify-center font-bold text-base md:text-lg tabular-nums ${metrics.streakType === 'win' ? 'border-emerald-400 text-emerald-500 dark:border-emerald-500/30' : metrics.streakType === 'loss' ? 'border-rose-400 text-rose-500 dark:border-rose-500/30' : 'border-slate-300 text-slate-500 dark:border-slate-700'}`}>
            {metrics.currentStreak}
          </div>
          <div className={`text-[10px] md:text-xs font-bold uppercase ${metrics.streakType === 'win' ? 'text-emerald-500' : metrics.streakType === 'loss' ? 'text-rose-500' : 'text-slate-400 dark:text-slate-500'}`}>
